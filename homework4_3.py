...
batchsize=100
learning_rate = 0.01
loss_rate_twonode = 0.1
learn_size=X.shape[0]
test_size=X2.shape[0]
or_initilize = input("学習済みの重みを使いますか？ (y/n): ")
if or_initilize == 'y':
...
def onenode(x_noramalized):
    ...
    y_onenode =np.maximum(0,t_onenode)
    return y_onenode

def dropout(x,rad_array,rate):
    dropout_array = (rad_array > rate).round().astype(int) * x
    return dropout_array

def dropout_test(x,rate):
    dropout_array =  x * (1 - rate)
    return dropout_array
...
def back_propagation_relu(y,delta_exp_y):
    delta_exp_relu = (y>0) * delta_exp_y
    return delta_exp_relu

def delta_dropout (delta_exp_y,rad_array,rate): 
    delta_dropout_array = (rad_array > rate).round().astype(int) * delta_exp_y
    return delta_dropout_array
...
for j in range(100):
    ...
    for i in range(learn_size//batchsize):
        ...
        random_array = np.random.rand(node_number,batchsize)
        ...
        y_onenode_dropped = dropout(y_onenode,random_array,loss_rate_twonode)
        y_twonode = twonode(y_onenode_dropped)
        ...
        delta_exp_w2 = back_propagation_w(delta_exp_a,y_onenode_dropped)
        delta_exp_b2 = back_propagation_b(delta_exp_a)
        delta_exp_dropout = delta_dropout(delta_exp_x2,random_array,loss_rate_twonode)
        delta_exp_relu = back_propagation_relu(y_onenode,delta_exp_dropout)
        delta_exp__w1 = back_propagation_w(delta_exp_relu,x_normalized)
        delta_exp_b1 = back_propagation_b(delta_exp_relu)
    ...
    for i in range(test_size//batchsize):
        ...
        x_normalized = minibatch.reshape(batchsize,size).T /256.0
        y_onenode = onenode(x_normalized)
        y_onenode_dropped = dropout_test(y_onenode,loss_rate_twonode)
        y_twonode = twonode(y_onenode_dropped)
        y_max = np.argmax(y_twonode,axis=0)
